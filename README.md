# Thiago Brabo

**São Paulo, SP** · thiago.dbrabo@gmail.com · [LinkedIn](https://www.linkedin.com/in/thiago-brabo-76110575/)

---
## Resumo profissional
Profissional com 15+ anos em Infraestrutura, DevOps e SRE, com atuação em ambientes críticos e de alta disponibilidade. Forte liderança técnica, visão sistêmica e foco em excelência operacional, automação e melhoria contínua.

---
## Indicadores do Git
<!-- Badges discretas (live) -->
![Último commit](https://img.shields.io/github/last-commit/thiagobrabo/eu-eu-mesmo?style=flat-square)
![Contribuidores](https://img.shields.io/github/contributors/thiagobrabo/eu-eu-mesmo?style=flat-square)
![PRs (abertas)](https://img.shields.io/github/issues-pr/thiagobrabo/eu-eu-mesmo?style=flat-square)
![Stars](https://img.shields.io/github/stars/thiagobrabo/eu-eu-mesmo?style=flat-square)

Resumo numérico (posso preencher automaticamente):
- Commits: [COMMIT_COUNT]
- Pull requests (total): [PR_COUNT]
- Pull requests por você: [PR_BY_YOU]
- Reviews realizadas: [REVIEWS_BY_YOU]
- Contributors: [CONTRIBUTORS_COUNT]
- Repositórios públicos (sua conta): [PUBLIC_REPOS_COUNT]

---
## Competências Técnicas
- Cloud: AWS · GCP · Azure · Oracle Cloud  
- Containers & Orquestração: Docker · Kubernetes · Helm · Istio  
- IaC: Terraform · Ansible · Packer · OpenTofu · Pulumi  
- CI/CD & GitOps: GitHub Actions · GitLab CI · ArgoCD  
- Observabilidade: Prometheus · Grafana · ELK · Datadog · New Relic  
- Linguagens/Script: Python · Bash · PowerShell  
- Mensageria: Kafka · RabbitMQ · Pub/Sub  
- Ferramentas: Git · SonarQube · Vault · Secret Managers · FinOps tools

---
## Experiência Profissional

### DevOps Team Lead — Envision Tecnologia (out/2022 – atual)
- Sustentação e desenvolvimento de ambientes WEB/API (.NET Core, React, Angular, Wordpress) em Azure, GCP, Cloudflare, DigitalOcean e on‑premises.
- Arquitetura e boas práticas focadas em entrega contínua, IaC, PCI e FinOps.
- Criação e liderança de time de plataforma para acelerar onboarding de devs e produtos.

### DevOps Senior — Octadesk (abr/2021 – nov/2022)
- Squads multidisciplinares, observabilidade, IaC e testes automatizados.
- Implementação de KEDA para autoscaling reativo a eventos.

### Líder de Infraestrutura — Envision Tecnologia (jan/2016 – abr/2021)
- Liderança do time de infraestrutura; arquitetura, segurança e governança; projetos de migração e PCI.

(Resumo das demais posições mantido — detalhe sob demanda)

---
## Formação
- Tecnólogo em Redes de Computadores — Faculdade Eniac (2014)  
- Tecnólogo em Administração e Negócios — Faculdades Integradas Torricelli (2007)

## Certificações
MCSDT · MCSA · Oracle Cloud Associate Foundations · Cursos em Cloud, DevOps, Observabilidade e FinOps

## Idiomas
Português (nativo) · Inglês (técnico) · Espanhol (técnico)
